...
import pickle
from OPS2NetUtils2 import getStatus2, getClassif,getCitations, getFamilyLenght, quote, UnNest, GenereDateLiens
...
#                appariement[change(j)+'-'+change(i)] = [j,i]
         ...
         #appariement['Country-tech'] = ['Country','tech']
...
#        appariement['inventor-IPCR4'] = ['inventeur','IPCR4']
        appariement['IPCR7-IPCR7'] = ['IPCR7','IPCR7']
...
#        try:
#            os.remove(ResultPathGephi+'\\'+ndf.replace('.dump', '')+"_CountryCrossTech"+'.gexf')
#        except:
#            pass
#            
#        os.rename(ResultPathGephi+'\\'+"Good"+ndf+'2.gexf', ResultPathGephi+'\\'+ndf.replace('.dump', '')+"_CountryCrossTech"+'.gexf')
#        print "Network file writen in ",  ResultPathGephi+' directory.\n See file: '+ndf.replace('.dump', '')+"_CountryCrossTech"+ndf.replace('.dump', '')+'.gexf'
#        
  ...
  #      import forceatlas
        ...
        #pos = nx.spring_layout(G, dim=2, k=0.2, scale =1, iterations = 50000) 
        pos=nx.graphviz_layout(G,prog='sfdp', args='-Goverlap="scale" -Gsize="1000,700" -GK=20' )
