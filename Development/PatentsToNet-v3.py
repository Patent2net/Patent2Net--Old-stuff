...
#    appariement[''] = ['','']
    ...
    lstIPC = ['IPCR1', 'IPCR3', 'IPCR4', 'IPCR7', 'IPCR11']
    for i in lstIPC:
        for j in lstIPC:
            if i == j: #only same IPC level
                appariement[change(i)+'-'+change(j)] = [i,j]
    for i in lstCrit:
        for j in lstIPC: #cross technology networks
            appariement[change(i)+'-'+change(j)] = [i,j]
...
#    appariement['inventor-IPCR11'] = ['inventor','IPCR11']
    ...
    for lien in reseau:
        ...
        try:
            ...
            print
    print "nombre d'exceptions pour :", contretps #this number prints exceptions due to inconsistent data
	# for instance no IPCR or no status data for a patent (here detected in links...)
    attr = dict() # dictionnary of links attributes
    import datetime
    ...
    #                    attr['url'] = "http://web2.wipo.int/ipcpub#lang=fr&menulang=FR&refresh=symbol&notion=scheme&version=20140101&symbol="+noeud[0:4]
                ...
                attr['status'] = getStatus (noeud, ListeBrevet)
...
##############################
                #hierachic dependency for IPC nodes :
################################                
