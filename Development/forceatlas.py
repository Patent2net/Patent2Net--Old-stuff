...
def forceatlas2_layout(G, iterations=10, linlog=False, pos=None, nohubs=False,
                       kr=0.001, k=None, dim=2, avoidoverlap = True):
    """
    Options values are
...
#        G.node[n]['currcs'] = 0
    ...
    if pos is None:
        ...
        try:
            ...
            pipo = np.asarray(np.random.random((nnodes, dim)), dtype=A.dtype)
            for ind in pos.keys():
                pipo[ind] = np.asarray(pos[ind])
            pos = pipo
        # # hack here
    if k is None:
    ...
    for iteration in range(iterations):
        ...
        for i in range(A.shape[0]):
            ...
            distance = np.where(distance < .01, .01, distance)
